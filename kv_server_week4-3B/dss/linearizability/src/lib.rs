...
pub fn check_operations_timeout<M: Model>(
    ...
    //println!("check_operations_timeout1");
    let partitions = model.partition(history);
    //println!("check_operations_timeout2");

    let (tx, rx) = channel();
    let mut handles = vec![];
    let kill = Arc::new(AtomicBool::new(false));
    let count = partitions.len();
    //println!("check_operations_timeout3");
    ...
    //println!("check_operations_timeout4");
    let res = wait_res(rx, kill, count, timeout);
    for handle in handles {
        handle.join().unwrap();
    }
    //println!("check_operations_timeout5");
    res
}
