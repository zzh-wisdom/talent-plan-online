...
pub struct ApplyMsg {
    ...
    /// mark whether it is a snapshot applying
...
pub struct RaftState {
    ...
    #[prost(int64, tag = "2")]
    pub vote_for: i64,
    /// the last log's index contained in snapshot
    #[prost(uint64, tag = "3")]
    pub snapshot_last_index: u64,
    /// the last log's term contained in snapshot
    #[prost(uint64, tag = "4")]
    pub snapshot_last_term: u64,
    /// Log vector
    #[prost(bytes, repeated, tag = "5")]
    pub logs: Vec<Vec<u8>>, //日志内容
}
...
pub struct Raft {
    ...
    /// In actual compression, the last committed log is not compressed
    /// leave it at the index 0,
    /// which is convenient for obtaining information such as pre_log_term.              

    /// The index of the last log of the snapshot
    pub snapshot_last_index: u64,
    /// The term of the last log of the snapshot
    pub snapshot_last_term: u64,
}
...
impl Raft {
    ...
    pub fn set_role(&mut self, is_leader: bool, is_follower: bool) {
        ...
        self.state = Arc::new(new_state);
        if is_leader {
            // Initialize as the last index of the snapshot
            self.next_index = Some(vec![self.snapshot_last_index + 1; self.peers.len()]);
    ...
    pub fn get_vote_for(&self) -> Option<usize> {
        self.vote_for
    }
    /// Set `voted_for`
    ...
    pub fn get_last_log_index(&self) -> usize {
        self.snapshot_last_index as usize + self.log.len() - 1
    }
    /// Get the term of the last log entry
    pub fn get_last_log_term(&self) -> u64 {
        let log = self
            .get_log(self.snapshot_last_index + self.log.len() as u64 - 1)
            .unwrap();
        log.term
    }
    ...
    pub fn get_log_vec(&self, index: u64) -> Option<Vec<u8>> {
        ...
        let index = index - self.snapshot_last_index;
    ...
    pub fn push_log(&mut self, index: u64, term: u64, entry: &Vec<u8>) {
        if self.snapshot_last_index + self.log.len() as u64 != index {
            my_debug!(
    ...
    pub fn handle_append_entries_reply(
        ...
        if result && needed_index > next_index[id] {
        ...
        } else if !result && check_index == next_index[id] {
            ...
            if (needed_index > self.snapshot_last_index + 1)
                && needed_index <= (self.get_last_log_index() as u64 + 1)
            {
                // Determine if the log of the `needed_index` location is compressed
                next_index[id] = needed_index;
            } else if next_index[id] < (SEND_ENTRIES_MAX + 1) {
                // Roll back next_index[id]
                next_index[id] = 1;
            } else {
                // // Roll back next_index[id]
        ...
        if old_match < match_index[id] {
            ...
            n_commit_index = match_index[(self.get_peers_len() - 1) / 2];
            self.set_commit_index(n_commit_index);
        }
        if n_commit_index != 0 {
            let log = self.get_log(n_commit_index);
            if log.is_none() {
                my_debug!(
                    "leader: {} get log none n_commit_index:{} snapshot_index:{}",
                    self.me,
                    n_commit_index,
            ...
            if log.term != self.term() {
                ...
                my_debug!(
                    "Error: Leader id:{} new_commit_index:{} log_term:{} != now_term:{}",
                    self.me,
                    n_commit_index,
                    log.term,
                    self.term()
    ...
    pub fn set_commit_index(&mut self, new_commit_index: u64) {
        ...
        let log = self.get_log(new_commit_index);
        if log.is_none() {
            my_debug!(
                "leader: {} get log none n_commit_index:{} snapshot_index:{}",
                self.me,
    ...
    pub fn update_applied(&mut self) {
        if self.commit_index > self.last_applied {
            ...
            for i in last..high {
                self.last_applied += 1;
                let log = self.log[(self.last_applied - self.snapshot_last_index) as usize].clone();
    ...
    /// Delete the logs before `index`, but not including `index`
    pub fn delete_prev_log(&mut self, index: u64) {
        let index = index - self.snapshot_last_index;
        if ((self.log.len() - 1) as u64) < index {
            //log is not enough
            my_debug!(
                "Error:id:{} delete_prev_log index:{} snapshot_last_index:{} log:{}",
                self.me,
                index,
                self.snapshot_last_index,
                self.log.len()
        ...
        let _delete: Vec<Vec<u8>> = self.log.drain(..(index as usize)).collect();
        for de in &_delete {
            let log: Log = match labcodec::decode(&de) {
                Ok(l) => l,
                Err(_) => continue,
    ...
    /// Compress logs, Upper layer call
    /// `index` is the index of the last log needed to be compressed
    pub fn compress_log(&mut self, maxraftstate: usize, index: u64) {
        my_debug!(
            "id:{} maxraftstate:{} raft_state.len:{} index:{}",
            self.me,
            maxraftstate,
        ...
        if maxraftstate > self.persister.raft_state().len() {
            //needn't to be compress
            return;
        }
        // The last submitted log was not deleted, placed in the 0 index position
        if index > self.commit_index || index <= self.snapshot_last_index {
            my_debug!(
                "Error:id:{} compress_log  index:{} commit:{} snapshot_last_index:{}",
                self.me,
                index,
                self.commit_index,
                self.snapshot_last_index
        ...
        // Delete the log before the `index`
        self.delete_prev_log(index);
        self.snapshot_last_index = index;
        my_debug!(
            "id:{} compress snapshot_last_index:{}",
            self.me,
            self.snapshot_last_index
        );
        let log = self.get_log(self.get_last_log_index() as u64).unwrap();
        self.snapshot_last_term = log.term;
        // No need to save the snapshot, it has been saved in kv_server
        self.persist();
    }
    /// Save raft state and snapshot
    pub fn save_state_and_snapshot(&self, snapshot_bytes: Vec<u8>) {
        let mut vote_for: i64 = -1;
        if self.vote_for.is_some() {
            vote_for = self.vote_for.clone().unwrap() as i64;
        }
        ...
        self.persister
            .save_state_and_snapshot(state_bytes, snapshot_bytes);
    ...
    fn persist(&mut self) {
        ...
        // The log with the subscript 0 at this time is useful and needs to be saved.
        for i in 0..self.log.len() {
            let log = self.log[i].clone();
            raft_state.logs.push(log);
        }
    ...
    fn restore(&mut self, data: &[u8]) {
        ...
        match labcodec::decode(data) {
            Ok(state) => {
                ...
                self.state = Arc::new(new_state);
                self.commit_index = state.snapshot_last_index;
                self.last_applied = state.snapshot_last_index;
                self.snapshot_last_index = state.snapshot_last_index;
                self.snapshot_last_term = state.snapshot_last_term;

                if state.vote_for == -1 {
                    self.vote_for = None;
                } else {
                    self.vote_for = Some(state.vote_for as usize);
                }
                // clear log
                self.log = vec![];

                my_debug!(
                    "Restore id:{} log_len:{} term:{} snapshot_last_index:{} snapshot_last_term:{}",
                    self.me,
                    state.logs.len(),
                    state.term,
        ...
        M: labcodec::Message,
    {
        // Your code here (2B).
        if self.is_leader() {
            let index = self.snapshot_last_index + self.log.len() as u64;
            let term = self.term();
            let mut buf = vec![];
...
impl Node {
    ...
    pub fn send_entries(node: Node) {
        ...
        for i in 0..len {
            ...
            if prev_log_index < raft.snapshot_last_index {
                //send snapshot
                let args = SnapshotArgs {
                    term: raft.term(),
                    leader_id: id as u64,
                    last_included_index: raft.snapshot_last_index,
                    last_included_term: raft.snapshot_last_term,
                    snapshot: raft.persister.snapshot(),
                };
                my_debug!("leader:{} send snapshot to:{} args:[term:{} snapshot_last_index:{} snapshot_last_term:{}] ", 
                ...
                peer.spawn(
                    peer.install_snapshot(&args).map(move |ret| {
                            ...
                            if ret.term > raft.term() {
                                ...
                                let _ret =
                                    node.timeout_sender.lock().unwrap().clone().unwrap().send(1);
                                raft.set_term(ret.term);
                                raft.set_role(false, true);
                                ...
                                let time2 = time.elapsed().as_millis();
                                my_debug!(
                                    "leader:[id:{} term:{}] get snapshot reply from:{} higher term:{} time:{}!!!",
                                    me,
                                    raft.term(),
                                    i,
                                    rep.term,
                                ...
                                // If it is still a leader, update next_index, match_index
                                ...
                                raft.match_index = Some(match_index);
                                let time2 = time.elapsed().as_millis();
                                my_debug!(
                                    "leader:{} send snapshot to:{} success last_included_index:{} time:{}! ",
                                    me,
                                    i,
                                    args.last_included_index,
                                    time2
                                );
                                my_debug!("leader:{} set next_index[{}]:{}", me, i, next_index[i]);
                            }
                    }).map_err(|_|{})
                );
                continue;
                ...
                peer.append_entries(&args).map(move |ret| {
                    ...
                    if ret.term > raft.term() {
                        ...
                        if ret.entries_granted {
                            raft.handle_append_entries_reply(i,true,ret.next_index,ret.check_index);
                            let time2 = time.elapsed().as_millis();
                            my_debug!("leader:{} get logs_granted:{} true time:{}!", me, i, time2);
                        } else {
                            raft.handle_append_entries_reply(i,false,ret.next_index,ret.check_index);
                            let time2 = time.elapsed().as_millis();
                            my_debug!(
    ...
    // For kvserver call
    pub fn compress_log(&self, maxraftstate: usize, index: u64) {
        self.raft.lock().unwrap().compress_log(maxraftstate, index);
    }
...
impl RaftService for Node {
    ...
    fn append_entries(&self, args: RequestEntriesArgs) -> RpcFuture<RequestEntriesReply> {
        my_debug!(
            "ID:{} Receive entries from id:{} pre_term:{}, pre_index:{}",
            self.me,
            args.leader_id,
            args.prev_log_term,
            args.prev_log_index
        ...
        let mut reply = RequestEntriesReply {
            ...
            next_index: 0,
            check_index: args.prev_log_index + 1,
        };
        let current_term = raft.term();
        if args.term < current_term {
            ...
            // leader is valid
            let _re = self.timeout_sender.lock().unwrap().clone().unwrap().send(1);
            if args.prev_log_index < raft.snapshot_last_index {
                // Smaller than the last index of the snapshot, the default match is successful
                reply.next_index = args.prev_log_index + args.entries.len() as u64 + 1;
                if (reply.next_index - 1) <= raft.snapshot_last_index {
                    // All indexed logs have been compressed and the default match is successful.
                    my_debug!(
                        "follower:{} lower last_index:{} snapshot_last_index:{}",
                        raft.me,
                        reply.next_index - 1,
                        raft.snapshot_last_index
                    );
                    reply.entries_granted = true;
                } else {
                    reply.next_index = raft.snapshot_last_index + 1;
                }
                my_debug!("Warning follower:{} term:{} snapshot_last_index:{} receive args:[pre_term:{} pre_index:{} log_len:{}], reply.next_index:{}", 
                    raft.get_id(), raft.term(), raft.snapshot_last_index, args.prev_log_term, args.prev_log_index, args.entries.len(), reply.next_index);
            } else {
                // Otherwise，match logs
                ...
                match entries {
                    Some(en) => {
                        ...
                        if en.term == args.prev_log_term {
                            ...
                            //let new_commit_index: u64 = cmp::min(args.leader_commit, raft.get_last_log_index() as u64); //取最小值
                            //上面的处理是错误的。有可能前面部分全部匹配，而后面未发过来的日志是未匹配的
                            let new_commit_index: u64 =
                                cmp::min(args.leader_commit, reply.next_index - 1); //取最小值
    ...
    fn install_snapshot(&self, args: SnapshotArgs) -> RpcFuture<SnapshotReply> {
        let mut raft = self.raft.lock().unwrap();
        let mut reply = SnapshotReply { term: raft.term() };
        if args.term < raft.term() {
            my_debug!(
                "Warning:follower[{}:{}] receive snapshot leader[{}:{}]",
                raft.me,
                raft.term(),
                args.leader_id,
                args.term
            );
        } else if args.last_included_index <= raft.snapshot_last_index {
            my_debug!(
                "warning:follower[{}:{}] receive snapshot leader[{}:{}]",
                raft.me,
                raft.snapshot_last_index,
            ...
            // Admit it as a leader, send a message to prevent timeouts
            let _ret = self.timeout_sender.lock().unwrap().clone().unwrap().send(1);
            let log = Log {
            ...
            let _ret = labcodec::encode(&log, &mut log_bytes).map_err(Error::Encode);
            if args.last_included_index >= (raft.snapshot_last_index + raft.log.len() as u64) {
                // All logs are replaced
                my_debug!("follower:{} all", raft.me);
                raft.log = vec![log_bytes];
            } else {
                my_debug!("follower:{} part", raft.me);
                raft.delete_prev_log(args.last_included_index);
                // attention !!!!!!!
                raft.log[0] = log_bytes;
            }
            raft.snapshot_last_index = args.last_included_index;
            raft.snapshot_last_term = args.last_included_term;
            ...
            let entries = raft.get_log(raft.snapshot_last_index);
            match entries {
                Some(en) => {
                    my_debug!(
                        "follower:{} log index:{} term:{}",
                        raft.me,
                        en.index,
                        en.term
                    );
            ...
            let mesg = ApplyMsg {
                command_valid: true,
                command: args.snapshot.clone(),
                command_index: raft.snapshot_last_index,
                is_snapshot: true,
            };
            // Send the snapshot to kv_server
